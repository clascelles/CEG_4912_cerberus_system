...
public class JsonMessageDecodingTest {
	...
	private static final String RFID_AUTH_RES_JSON_MESSAGE = "{ \"type\" : \"RFID_AUTH_RES\", \"socketId\" : 12345, \"timestamp\" : 1363702072, \"rfidNumber\" : 1234567890, \"auth\" : true}";
	...
	public void testDecodingJsonCurrentConsumptionMessage() throws Exception {
		...
		assertThat(currentMsg.getTimestamp().getMillis(), is(equalTo(timestamp)));
		assertThat(currentMsg.getType(), is(equalTo(Message.MessageType.CURRENT)));
	...
	public void testDecodingJsonRFIDAuthRequestMessage() throws Exception {
		...
		assertThat(rfidMsg.getTimestamp().getMillis(), is(equalTo(timestamp)));
		assertThat(rfidMsg.getType(), is(equalTo(Message.MessageType.RFID_AUTH_REQ)));
	...
	public void testDecodingJsonRFIDAuthResponseMessage() throws Exception {
		...
		assertThat(rfidMsg.getSocketId(), is(equalTo(socketId)));
		assertThat(rfidMsg.getTimestamp().getMillis(), is(equalTo(timestamp)));
		assertThat(rfidMsg.getType(), is(equalTo(Message.MessageType.RFID_AUTH_RES)));
		assertThat(rfidMsg.getRfidNumber(), is(equalTo(1234567890)));
	...
	public void testDecodingJsonStatusMessage() throws Exception {
		...
		assertThat(currentMsg.getSocketId(), is(equalTo(socketId)));
		assertThat(currentMsg.getTimestamp().getMillis(), is(equalTo(timestamp)));
		assertThat(currentMsg.getType(), is(equalTo(Message.MessageType.STATUS)));
	...
	public void testDecodingJsonSwitchOperatingModeMessage() throws Exception {
		...
		assertThat(switchMsg.getTimestamp().getMillis(), is(equalTo(timestamp)));
		assertThat(switchMsg.getType(), is(equalTo(Message.MessageType.SWITCH_OP_MODE)));
	...
	public void testDecodingJsonGenericInformationMessage() throws Exception {
		...
		assertThat(genericMsg.getTimestamp().getMillis(), is(equalTo(timestamp)));
		assertThat(genericMsg.getType(), is(equalTo(Message.MessageType.GENERIC_INFO)));
